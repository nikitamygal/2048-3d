...
namespace ProceduralPrimitivesUtil
{
    public class PPCombiner : PPBase
    {
        ...
        public bool applyElementsTransform = true;
        ...
        protected override void CreateMesh()
        {
            ...
            {
                if (go == gameObject || go == null) continue;
                if (includeChildren)
                {
                    ...
                    foreach (var mf in mfs)
                    {
                        ...
                        if (applyElementsTransform) AddMesh(mf.sharedMesh, mf.transform);
        ...
        private void AddMesh(Mesh m, Transform trans = null)
        {
            ...
            {
                triangles[i] += m_builder.m_vertices.Count;
            }

            m_builder.m_vertices.AddRange(vertices);
            m_builder.m_normals.AddRange(normals);
            m_builder.m_triangles.AddRange(triangles);
            m_builder.m_uv.AddRange(m.uv);
        }
    }
}